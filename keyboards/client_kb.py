...
def product_list_kb(products: list, category_id=None):
    ...
    back_cb = f"cat_{category_id}" if category_id else "catalog"
...
def cart_kb(items: list):
    ...
    for item in items:
        buttons.append([
            InlineKeyboardButton(text=f"➖", callback_data=f"cart_minus_{item['product_id']}"),
            InlineKeyboardButton(text=f"{item['name'][:20]} x{item['qty']}", callback_data="noop"),
            InlineKeyboardButton(text=f"➕", callback_data=f"cart_plus_{item['product_id']}"),
...
def order_detail_client_kb(order_id: int, has_invoice: bool, invoice_requested: bool):
    ...
    if has_invoice:
        ...
        buttons.append([InlineKeyboardButton(text="📄 Запросити рахунок", callback_data=f"req_invoice_{order_id}")])
    else:
